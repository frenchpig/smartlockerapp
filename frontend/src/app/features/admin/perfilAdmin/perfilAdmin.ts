...
@Component({
  ...
  imports: [CommonModule, RouterModule, ReactiveFormsModule, HeaderAdmin],
  ...
  async loadUserProfile() {
    ...
    try {
      const profile = await this.http
        .get<UserProfile>(`${environment.apiUrl}/usuarios/${this.auth.user()?.id}`)
        .toPromise();
  ...
  async guardarPerfil() {
    ...
    try {
      const updated = await this.http
        .patch<UserProfile>(`${environment.apiUrl}/auth/profile`, this.editForm.value)
        .toPromise();
